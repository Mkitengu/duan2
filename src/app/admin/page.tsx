...
export default function AdminPage() {
  ...
  const [dispatchCombinedOrders, setDispatchCombinedOrders] = useState<ServerOrder[]>([]);
  const [combinedDispatchMessage, setCombinedDispatchMessage] = useState("");
  const [copiedCombinedInvoice, setCopiedCombinedInvoice] = useState(false);
...
Cảm ơn bạn đã ủng hộ quán! Đồ uống đang được giao đến bạn.`;
  ...
  const handleBatchApproveAndCombine = async () => {
    ...
    try {
      // 1. Approve all selected orders that are not already approved
      const pendingApprovals = selectedOrders.filter(o => o.status !== "approved");
      if (pendingApprovals.length > 0) {
        ...
        setOrders(prevOrders =>
          prevOrders.map(o =>
            selectedOrderIds.includes(o.id) ? { ...o, status: "approved" } : o
          )
        );
      }

      // 2. Generate combined invoice text
      const mergedItemsMap: { [name: string]: { quantity: number; price: number } } = {};
      selectedOrders.forEach(order => {
        order.items.forEach(item => {
          if (mergedItemsMap[item.name]) {
            mergedItemsMap[item.name].quantity += item.quantity;
            mergedItemsMap[item.name].price = Math.max(mergedItemsMap[item.name].price, item.price);
          } else {
            mergedItemsMap[item.name] = { quantity: item.quantity, price: item.price };
          }
        });
      });

      const mergedItems = Object.entries(mergedItemsMap).map(([name, detail]) => ({
        name,
        quantity: detail.quantity,
        price: detail.price,
      }));

      const customerDetailsLines = selectedOrders.map((o, idx) => {
        return `${idx + 1}. ${o.customer.name} - ${o.customer.phone} (${o.customer.address})${o.customer.note ? ` [Ghi chú: ${o.customer.note}]` : ""}`;
      }).join("\n");

      const totalSum = selectedOrders.reduce((sum, o) => sum + o.total, 0);
      const orderIdsList = selectedOrders.map(o => o.id).join(", ");

      const invoiceMsg = `HÓA ĐƠN GỘP (${selectedOrders.length} ĐƠN HÀNG)
====================
Mã các đơn: ${orderIdsList}
Khách hàng đã gộp:
${customerDetailsLines}

Sản phẩm tổng hợp:
${mergedItems.map(item => `• ${item.name} x${item.quantity} = ${formatCurrency(item.price * item.quantity)}`).join("\n")}

Tổng bill thanh toán: ${formatCurrency(totalSum)}
====================
Cảm ơn quý khách đã ủng hộ quán! Đồ uống đang được giao đến các bạn.`;

      setDispatchCombinedOrders(selectedOrders);
      setCombinedDispatchMessage(invoiceMsg);
      setCopiedCombinedInvoice(false);
      setSelectedOrderIds([]);
    } catch (error) {
      console.error("Failed to batch approve & combine:", error);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 antialiased font-sans">
      ...
      {!isAuthenticated ? (
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
          ...
          {/* Orders Counter Banner & Select All */}
          <div className="flex items-center justify-between mb-4 ml-1">
            ...
            {filteredOrders.length > 0 && (
              <button
                ...
                className="flex items-center gap-2 text-xs font-bold text-teal-600 hover:text-teal-700 bg-teal-50/50 border border-teal-100 hover:bg-teal-50 px-3 py-1.5 rounded-xl transition-all"
              >
                <input
                  type="checkbox"
                  checked={
                  ...
                  className="w-3.5 h-3.5 rounded border-slate-300 text-teal-600 focus:ring-teal-500 cursor-pointer accent-teal-500"
                />
      ...
      {/* ========== FLOATING ACTION BAR FOR BATCH APPROVAL ========== */}
      <AnimatePresence>
        {selectedOrderIds.length > 0 && (
          ...
          >
            <div className="bg-slate-900/95 backdrop-blur-md border border-slate-800 rounded-3xl p-4 shadow-2xl flex items-center justify-between gap-4 text-white">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-teal-500/20 text-teal-400 flex items-center justify-center font-bold">
                  {selectedOrderIds.length}
                </div>
                <div>
                  <p className="text-sm font-bold">Đã chọn {selectedOrderIds.length} đơn hàng</p>
                  <p className="text-[10px] text-slate-400 font-medium">Bạn có thể duyệt & gộp chung thành một hóa đơn</p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold transition-all uppercase tracking-wider"
                >
                  Hủy chọn
                </button>
                <button
                  onClick={handleBatchApproveAndCombine}
                  disabled={isProcessingBatch}
                  className="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-teal-500 to-blue-500 text-white text-xs font-black shadow-lg shadow-teal-500/20 hover:shadow-teal-500/35 transition-all flex items-center gap-1.5 uppercase tracking-wider disabled:opacity-50"
                >
                  {isProcessingBatch ? (
                    <>
      ...
      {/* ========== POPUP 3: COMBINED INVOICE MODAL ========== */}
      <AnimatePresence>
        {dispatchCombinedOrders.length > 0 && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            {/* Backdrop */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setDispatchCombinedOrders([])}
              className="fixed inset-0 bg-slate-900/60 backdrop-blur-md"
            />
            ...
            <motion.div
              ...
              exit={{ scale: 0.94, y: 20, opacity: 0 }}
              transition={{ type: "spring", damping: 25, stiffness: 350 }}
              className="bg-white border border-slate-100 rounded-3xl w-full max-w-lg shadow-2xl p-6 relative overflow-hidden z-10 flex flex-col max-h-[92vh]"
            >
              {/* Glow effects inside card */}
              <div className="absolute -top-12 -right-12 w-32 h-32 bg-teal-500/10 rounded-full blur-2xl pointer-events-none" />
              <div className="absolute -bottom-12 -left-12 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl pointer-events-none" />
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100 relative z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-extrabold text-slate-800 text-base">
                      Đã duyệt & Gộp {dispatchCombinedOrders.length} hóa đơn
                    </h3>
                    <p className="text-[10px] text-slate-400 font-bold">
                      Gửi hóa đơn tổng hợp Zalo tới khách hàng hoặc quán
                    </p>
                  </div>
                </div>
                <button
              ...
              {/* Scrollable content */}
              <div className="flex-1 overflow-y-auto py-5 space-y-4 relative z-10 pr-1">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">
                      Nội dung hóa đơn gộp
                    </span>
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={async () => {
                        const success = await copyToClipboard(combinedDispatchMessage);
                        if (success) {
                          setCopiedCombinedInvoice(true);
                          setTimeout(() => setCopiedCombinedInvoice(false), 2000);
                        }
                      }}
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-extrabold transition-all ${
                        copiedCombinedInvoice
                          ? "bg-green-100 text-green-700"
                          : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                      }`}
                    >
                ...
                {/* Dispatch options */}
                <div className="space-y-3">
                  <span className="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">
                    Gửi Zalo cho từng số điện thoại khách hàng:
                  </span>
                  
                  <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                    {dispatchCombinedOrders.map((order) => (
                      <motion.button
                        key={order.id}
                        whileHover={{ scale: 1.01 }}
                        whileTap={{ scale: 0.99 }}
                        onClick={async () => {
                          await copyToClipboard(combinedDispatchMessage);
                          await openZaloDirect(order.customer.phone, combinedDispatchMessage);
                        }}
                        className="w-full py-2.5 px-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 hover:bg-teal-50/50 hover:border-teal-200 transition-all flex items-center justify-between text-xs font-bold"
                      >
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1 text-[10px] text-teal-600 font-bold">
                          Gửi Zalo <ExternalLink className="w-3 h-3" />
                        </div>
                      </motion.button>
                    ))}
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-100">
                    <motion.button
                      ...
                      onClick={async () => {
                        await copyToClipboard(combinedDispatchMessage);
                        await openZaloDirect(DEFAULT_SHOP_PHONE, combinedDispatchMessage);
                      }}
                      className="w-full py-3 px-4 rounded-2xl bg-[#0068FF] text-white font-bold text-xs shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 transition-all flex items-center gap-3"
                    >
                      <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center flex-shrink-0">
                        <Send className="w-4 h-4 text-white" />
                      </div>
                      <div className="text-left">
                        <p className="font-extrabold text-sm">Gửi Zalo cho Quán (Báo pha chế gộp)</p>
                        <p className="text-[10px] text-blue-200 mt-0.5 font-semibold">
                          Mở Zalo quán ({DEFAULT_SHOP_PHONE}) để pha chế toàn bộ
                        </p>
                      </div>
                      <ExternalLink className="w-4 h-4 ml-auto opacity-70 flex-shrink-0" />
                    </motion.button>
                ...
                <div className="bg-amber-50 border border-amber-200/50 rounded-2xl p-4">
                  <p className="text-[10px] font-bold text-amber-700 leading-relaxed">
                    📌 <strong>Quy trình gửi:</strong> Click nút gửi tương ứng → Hệ thống tự
                    động sao chép hóa đơn gộp và mở chat Zalo → Click vào khung
                    chat Zalo và nhấn tổ hợp phím <strong>Ctrl+V</strong> (hoặc nhấn giữ dán trên điện thoại) và gửi.
                  </p>
                </div>
              </div>

              {/* Close action */}
              <div className="border-t border-slate-100 pt-4 flex justify-end relative z-10 bg-white">
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setDispatchCombinedOrders([])}
